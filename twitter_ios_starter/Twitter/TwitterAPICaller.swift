...
class TwitterAPICaller: BDBOAuth1SessionManager {    
    static let client = TwitterAPICaller(baseURL: URL(string: "https://api.twitter.com"), consumerKey: "Vogprm6fm6Paq8J0Cdsh43y49", consumerSecret: "nEciQud0SM1XSEQY8EILAZUAmrS7BBp3M1QmjvqioNZItdfP14")
    var loginSuccess: (() -> ())?
    var loginFailure: ((Error) -> ())?
    //consumer key: dDBtdUFiNnk3Z2ZaMjB6STdJUEg6MTpjaQ
    //consumer secret:HdA7-mzKq_XtfpqCewJ_CKqQsK2n_IojBVadt7ow1u0ZjsuhdB
    ...
    func favoriteTweet(tweetId:Int, success: @escaping () -> (), failure: @escaping (Error)->()){
        ...
        TwitterAPICaller.client?.post(url, parameters: ["id":tweetId], progress: nil, success: { (task: URLSessionDataTask, response: Any?) in success() }, failure: { (task: URLSessionDataTask?, error: Error) in
            failure(error)  })
    }
